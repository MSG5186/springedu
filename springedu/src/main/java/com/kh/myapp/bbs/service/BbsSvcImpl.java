...
public class BbsSvcImpl implements BbsSvc {
	...
	public void list(HttpServletRequest request, Model model) throws Exception{
		...
		FindCriteria fc = null;					//검색조건이 있는경우의 레코드 시작,종료페이지연산 + 검색타입 + 검색어
		...
		if(keyword == null || keyword.trim().isEmpty()) {
			...
			fc = new FindCriteria(reqPage,NUM_PER_PAGE,searchType,keyword);
			...
			pc = new PageCriteria(fc,totalRec,PAGE_NUM_PER_PAGE);
	...
	public int SearchTotalRec(String searchType, String keyword) throws Exception {
		int searchtotalRec = 0;
		searchtotalRec = bbsdao.SearchTotalRec(searchType, keyword);
		return searchtotalRec;
	}
	@Override
